...
func init() {
	...
	logicDataMap.Objects["设备"] = &phome.ObjectLogic{
		Description: "对智能家居设备进行信息获取或者操作设备",
		Action:      []string{"操作设备", "修改设备信息", "查询设备状态", "查询设备数量"},
	}

	logicDataMap.Objects["任务"] = &phome.ObjectLogic{
		Description: "创建并执行定时、延时任务或者顺序任务",
		Action:      []string{"定时器", "顺序执行", "延时"},
	}

	logicDataMap.Objects["日常对话"] = &phome.ObjectLogic{
		Description: "并非针对智能家居所在领域的对话,每种类型相当于一个智能agent,默认是其他类型",
		Action:      []string{"其他"},
	}

	logicDataMap.Objects["天气"] = &phome.ObjectLogic{
		Description: "查询当地或者某个地区的天气",
		Action:      []string{"查询天气"},
	}

	logicDataMap.Objects["时间日期"] = &phome.ObjectLogic{
		Description: "对日期时间的查",
		Action:      []string{"查询天气"},
	}

	logicDataMap.Objects["函数调用"] = &phome.ObjectLogic{
		Description: "执行一些函数调用",
		Action:      []string{"初始化场景", "初始化自动化", "初始化灯具", "初始化开关", "初始化灯光效果", "初始化灯控自动化"},
	}
}

var logicDataMap *phome.AppointmentReq
