...
func QueryScene(message, aiMessage, deviceId string) string {
	...
	if strings.Contains(aiMessage, "query_number") {
		content = fmt.Sprintf(`这是我的全部场景信息%v，我计算好了总数是%d个，根据我的意图用简洁、人性化的语言回答我。`, x.MustMarshal2String(sendData), len(sendData))
	}
...
功能2:根据场景称统计自动化数量：例如："共有5个场景"`, x.MustMarshal2String(sendData))
	}
	...
	msg, err := chatCompletionInternal([]*chat.ChatMessage{
		{
			Role:    "system",
			Content: fmt.Sprintf(`请根据场景信息用简洁、人性化的语言且不超过50字回答我，当前场景信息配置如下：%s`, x.MustMarshal2String(scene)),
		},
		{
			Role:    "user",
			Content: message,
