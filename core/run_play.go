...
返回JSON格式：{"id":"设备id","content":"","need_position":false,"name":"目标人员名称","timing":0}`, x.MustMarshal2String(speaker), x.MustMarshal2String(speakerData))
...
返回JSON格式：{"content":""}`, time.Now().Format(time.DateTime), x.MustMarshal2String(speaker))
	} else {
		return "请告诉我你要发送消息给谁。"
	}
